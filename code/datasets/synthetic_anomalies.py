...
def patch_ex_guided(ima_dest, guided_mask, ima_src=None, same=False, num_patches=1,
                    mode=cv2.NORMAL_CLONE, width_bounds_pct=((0.05,0.2),(0.05,0.2)),
                    label_mode='binary', tol=1, resize=True,
                    gamma_params=None, intensity_logistic_params=(1/6, 20),
                    resize_bounds=(0.7, 1.3), verbose=True, cutpaste_patch_generation=False):
    """
    Guided variant of patch_ex that restricts paste locations to the provided guided_mask.
    """
    if guided_mask is None:
        raise ValueError("guided_mask must be provided for patch_ex_guided.")

    guided_mask = np.asarray(guided_mask)
    if guided_mask.ndim == 3:
        guided_mask = guided_mask[..., 0]
    if guided_mask.shape != ima_dest.shape[:2]:
        raise ValueError("guided_mask shape must match spatial dimensions of ima_dest.")
    guided_mask_bool = guided_mask > 0
    ...
    for i in range(num_patches):
        if i == 0 or np.random.randint(2) > 0:
            patchex, ((_coor_min_dim1, _coor_max_dim1), (_coor_min_dim2, _coor_max_dim2)), patch_mask = _patch_ex_guided(
                patchex, ima_src, mode, label_mode, resize, width_bounds_pct,
                gamma_params, factor, resize_bounds, verbose,
                cutpaste_patch_generation, guided_mask_bool)
            if patch_mask is not None:
                mask[_coor_min_dim1:_coor_max_dim1,_coor_min_dim2:_coor_max_dim2] = patch_mask
                coor_min_dim1 = min(coor_min_dim1, _coor_min_dim1)
                coor_max_dim1 = max(coor_max_dim1, _coor_max_dim1)
...
def _patch_ex_guided(ima_dest, ima_src, mode, label_mode, resize,
                     width_bounds_pct, gamma_params, factor, resize_bounds,
                     verbose, cutpaste_patch_generation, guided_mask):
    guided_mask = np.asarray(guided_mask).astype(bool)
    if guided_mask.shape != ima_dest.shape[:2]:
        raise ValueError("guided_mask must match ima_dest spatial dimensions.")

    mask_indices = np.column_stack(np.where(guided_mask))
    if mask_indices.size == 0:
        if verbose:
            print('Guided mask is empty; cannot place patch.')
        return ima_dest.copy(), ((0,0),(0,0)), None

    mask_min = mask_indices.min(axis=0)
    mask_max = mask_indices.max(axis=0)
    mask_height = mask_max[0] - mask_min[0] + 1
    mask_width = mask_max[1] - mask_min[1] + 1
    max_patch_dim1 = max(1, mask_height // 2)
    max_patch_dim2 = max(1, mask_width // 2)
    ...
    if cutpaste_patch_generation:
        ...
        patch_width_dim1 = int(np.rint(np.clip(np.sqrt(area_ratio * aspect_ratio * dims[0]**2), 0, dims[0])))
        patch_width_dim2 = int(np.rint(np.clip(area_ratio * dims[0]**2 / patch_width_dim1, 0, dims[1])))
        patch_width_dim1 = max(1, min(patch_width_dim1, max_patch_dim1))
        patch_width_dim2 = max(1, min(patch_width_dim2, max_patch_dim2))
        center_dim1 = np.random.randint(patch_width_dim1, dims[0] - patch_width_dim1)
        center_dim2 = np.random.randint(patch_width_dim2, dims[1] - patch_width_dim2)
        ...
        min_width_dim2 = (width_bounds_pct[1][0]*dims[1]).round().astype(int)
        max_width_dim2 = (width_bounds_pct[1][1]*dims[1]).round().astype(int)
        max_width_dim1 = max(1, min(max_width_dim1, max_patch_dim1))
        max_width_dim2 = max(1, min(max_width_dim2, max_patch_dim2))
        min_width_dim1 = max(1, min(min_width_dim1, max_width_dim1))
        min_width_dim2 = max(1, min(min_width_dim2, max_width_dim2))
        ...
        if gamma_params is not None:
            ...
            if min_width_dim1 >= max_width_dim1:
                patch_width_dim1 = max_width_dim1
            else:
                patch_width_dim1 = np.random.randint(min_width_dim1, max_width_dim1)
            if min_width_dim2 >= max_width_dim2:
                patch_width_dim2 = max_width_dim2
            else:
                patch_width_dim2 = np.random.randint(min_width_dim2, max_width_dim2)
    ...
    if patch_mask is not None:
        patch_positive = patch_mask[...,0] > 0
        if not np.any(patch_positive):
            patch_positive = np.ones_like(patch_positive, dtype=bool)
    else:
        patch_positive = None
    ...
    attempts = 0
    max_attempts = 500
    while attempts < max_attempts and not found_center:
        idx = np.random.randint(mask_indices.shape[0])
        center_dim1, center_dim2 = mask_indices[idx]

        coor_min_dim1 = center_dim1 - height//2
        coor_max_dim1 = center_dim1 + (height+1)//2
        coor_min_dim2 = center_dim2 - width//2
        coor_max_dim2 = center_dim2 + (width+1)//2

        if (coor_min_dim1 < 0 or coor_min_dim2 < 0 or
            coor_max_dim1 > ima_dest.shape[0] or coor_max_dim2 > ima_dest.shape[1]):
            attempts += 1
            continue

        guided_submask = guided_mask[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2]
        if guided_submask.shape != (height, width):
            attempts += 1
            continue

        if patch_positive is None or not np.any(patch_positive):
            coverage = np.mean(guided_submask)
        else:
            coverage = np.mean(guided_submask[patch_positive])
        if coverage < 0.50:
            attempts += 1
            continue
    ...
    if not found_center:
        if verbose:
            print('No suitable guided center found.')
        return ima_dest.copy(), ((0,0),(0,0)), None

    guided_window = guided_mask[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2]
    patch_mask = (patch_mask[...,0] * guided_window.astype(np.uint8))[...,None]
    if not np.any(patch_mask):
        if verbose:
            print('Guided window removed entire patch.')
        return ima_dest.copy(), ((0,0),(0,0)), None
